...
resource "null_resource" "build_save" {
  triggers = {
    always_run = "${timestamp()}"
  }

  provisioner "local-exec" {
    command = "bash ${path.module}/../code/save/build.sh"
  }
}
...
data "archive_file" "save" {
  type        = "zip"
  output_path = "${path.module}/../code/save.zip" # TODO: change here
  source_dir  = "${path.module}/../code/save"     # TODO: change here

  depends_on = [
    null_resource.build_save # TODO: change here
  ]
}

# Tells Terraform to create an AWS lambda function
# - Filename here corresponds to the output_path in archive_file.save.
...
resource "aws_lambda_function" "save" {
  ...
  function_name = "${var.group_name}_${terraform.workspace}_save" # TODO: change here
  handler       = "handler.handler"
  runtime       = "python3.9" # TODO: change here

  role             = aws_iam_role.iam_for_lambda.arn
  source_code_hash = data.archive_file.save.output_base64sha256 # TODO: change here
...
resource "aws_lambda_permission" "save" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.save.function_name # TODO: change here
  principal     = "apigateway.amazonaws.com"

  source_arn = "${data.aws_apigatewayv2_api.api_gateway_global.execution_arn}/*/*/save" # TODO: change here
}
...
resource "aws_apigatewayv2_integration" "save" {
  ...
  integration_uri    = aws_lambda_function.save.invoke_arn # TODO: change here
...
resource "aws_apigatewayv2_route" "save" {
  api_id    = var.gateway_api_id
  route_key = "POST /${var.group_name}/save" # TODO: change here

  target = "integrations/${aws_apigatewayv2_integration.save.id}" # TODO: change here
...
resource "aws_cloudwatch_log_group" "save_log" {
  name              = "/aws/lambda/${aws_lambda_function.save.function_name}" # TODO: change here
  retention_in_days = 7
  lifecycle {
