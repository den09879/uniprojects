...
resource "null_resource" "build_wind" {
  triggers = {
    always_run = "${timestamp()}"
  }

  provisioner "local-exec" {
    command = "bash ${path.module}/../code/wind/build.sh"
  }
}


# Tells Terraform to compress your source code with dependencies
data "archive_file" "wind" {
  type        = "zip"
  output_path = "${path.module}/../code/wind.zip" # TODO: change here
  source_dir  = "${path.module}/../code/wind"     # TODO: change here

  depends_on = [
    null_resource.build_wind # TODO: change here
  ]
}

# Tells Terraform to create an AWS lambda function
# - Filename here corresponds to the output_path in archive_file.wind.
...
resource "aws_lambda_function" "wind" {
  filename      = data.archive_file.wind.output_path
  function_name = "${var.group_name}_${terraform.workspace}_wind" # TODO: change here
  handler       = "newrelic_lambda_wrapper.handler"
  runtime       = "python3.9" # TODO: change here

  role             = aws_iam_role.iam_for_lambda.arn
  source_code_hash = data.archive_file.wind.output_base64sha256 # TODO: change here
...
resource "aws_lambda_permission" "wind" {
  statement_id  = "AllowAPIGatewayInvoke"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.wind.function_name # TODO: change here
  principal     = "apigateway.amazonaws.com"

  source_arn = "${data.aws_apigatewayv2_api.api_gateway_global.execution_arn}/*/*/wind" # TODO: change here
}
...
resource "aws_apigatewayv2_integration" "wind" {
  api_id           = var.gateway_api_id
  integration_type = "AWS_PROXY"

  integration_uri    = aws_lambda_function.wind.invoke_arn # TODO: change here
...
resource "aws_apigatewayv2_route" "wind" {
  api_id    = var.gateway_api_id
  route_key = "POST /${var.group_name}/wind" # TODO: change here

  target = "integrations/${aws_apigatewayv2_integration.wind.id}" # TODO: change here
...
resource "aws_cloudwatch_log_group" "wind_log" {
  name              = "/aws/lambda/${aws_lambda_function.wind.function_name}" # TODO: change here
  retention_in_days = 7
  lifecycle {
